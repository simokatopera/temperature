...
export function getSeasonTrendsTS(series: TemperatureMsg, monthnumbers: number[], monthnames: string[], monthlytrends: MonthDataPair[]): GraphSerie[] {
    ...
    let seriess = [];
    if (monthlytrends[monthnumbers[0] - 1]) seriess.push(monthlytrends[monthnumbers[0] - 1]);
    if (monthlytrends[monthnumbers[1] - 1]) seriess.push(monthlytrends[monthnumbers[1] - 1]);
    if (monthlytrends[monthnumbers[2] - 1]) seriess.push (monthlytrends[monthnumbers[2] - 1]);
