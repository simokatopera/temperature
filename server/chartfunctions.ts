...
// function createMonthAverageCounters(year: number): AverageMinMaxCalcValues[] {
//     let data: AverageMinMaxCalcValues[] = [];
//     for (let i = 0; i < 12; i++) data.push(createAverageYearCounter( year, i + 1));
//     return data;
// }
...
export function createDefaultYearTable(defaultyear: number): AverageMinMaxCalcValues [] {
...
class Temperatures {
    ...
    calculateYearlyAndMonthlyAverages(temperatures: TemperatureMsg): CalculationResult {
        ...
        const returnvalues: YearlyAverageData[] = temperatures.data.map(currentyear => {
        ...
        function updateCounters(dest: AverageMinMaxCalc, value: number, year: number, month: number): boolean {
        ...
        let monthlydata: FilterValue[] = monthcounters.map(counter => {
            return createFilterValue(new Date(this.defaultyear, counter.monthno-1, 1), 
                createAverageMinMaxCalculated(counter.average.count > 0 ? counter.average.sum/counter.average.count:NaN, counter.average.average.high, counter.average.average.highdate, counter.average.average.low, counter.average.average.lowdate),
    ...
    calculateDailyAverages(temperatures: TemperatureMsg, year: number = null): CalculationResult {
        ...
        const returnvalue: AverageCalculated[] = calculationtable.map(sum => {
            let morning: OneDayValues = createOneDayValues(sum.morning.count, sum.morning.average.value, { date: sum.morning.average.lowdate, value: sum.morning.average.low }, { date: sum.morning.average.highdate, value: sum.morning.average.high });
            let evening: OneDayValues = createOneDayValues(sum.evening.count, sum.evening.average.value, { date: sum.evening.average.lowdate, value: sum.evening.average.low }, { date: sum.evening.average.highdate, value: sum.evening.average.high });
            let difference: OneDayValues = createOneDayValues(sum.difference.count, sum.difference.average.value, { date: sum.difference.average.lowdate, value: sum.difference.average.low }, { date: sum.difference.average.highdate, value: sum.difference.average.high });
            let total: OneDayValues = createOneDayValues(sum.difference.count, sum.total.average.value, { date: sum.total.average.lowdate, value: sum.total.average.low }, { date: sum.total.average.highdate, value: sum.total.average.high });
